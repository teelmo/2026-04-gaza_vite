export async function parseSubtitles(url) {
  if (url) {
    const response = await fetch(url);
    const text = await response.text();
    const jsonSubtitles = parse(text, {});
  ...
  //if (jsonSubtitles) {
  //  const track = ref.addTextTrack("subtitles", "Finnish", "fi");
  //  track.mode = "showing";
  //  jsonSubtitles.cues.map((d) => {
  //    track.addCue(new VTTCue(d.start, d.end, d.text));
  //  });
  //}
}

export async function jsonToTrack(json, ref) {
  try {
    if (json) {
      //hide track if already exist
      const numTracks = ref.textTracks.length;
      for (let i = numTracks - 1; i >= 0; i--)
        ref.textTracks[i].mode = "disabled";
...
function parse(input, options) {
  ...
  if (typeof input !== "string") {
    throw new ParserError("Input must be a string", null);
  }
  ...
  if (!header.startsWith("WEBVTT")) {
    throw new ParserError('Must start with "WEBVTT"', null);
  }
  ...
  if (
    ...
    throw new ParserError("Header comment must start with space or tab", null);
  }
  ...
  if (!meta && headerParts.length > 1 && headerParts[1] !== "") {
    throw new ParserError("Missing blank line after signature", null);
  }
...
function parseCue(cue, i, strict) {
  ...
  if (lines.length === 1 && !lines[0].includes("-->")) {
    throw new ParserError(
      `Cue identifier cannot be standalone (cue #${i})`,
      null
    );
  }
  ...
  if (
    ...
    throw new ParserError(msg, null);
  }
  ...
  if (
    ...
    throw new ParserError(`Invalid cue timestamp (cue #${i})`, null);
  }
  ...
  if (strict) {
    if (start > end) {
      throw new ParserError(
        `Start timestamp greater than end (cue #${i})`,
        null
      );
    }

    if (end <= start) {
      throw new ParserError(`End must be greater than start (cue #${i})`, null);
    }
  }
