//original repo https://github.com/developit/snarkdown
const TAGS = {
	'': ['<em>','</em>'],
	_: ['<strong>','</strong>'],
	'*': ['<strong>','</strong>'],
...
export default function parse(md, prevLinks) {
	...
	while ( (token=tokenizer.exec(md)) ) {
		...
		// Links:
			...
			out = token[11].includes("yle.fi") 
            ? out.replace('<a>', `<a href="${encodeAttr(token[11] || links[prev.toLowerCase()])}">`)
            : out.replace('<a>', `<a href="${encodeAttr(token[11] || links[prev.toLowerCase()])}" target="_blank">`);
			chunk = flush() + '</a>';
		}
		else if (token[9]) {
			chunk = '<a>';
