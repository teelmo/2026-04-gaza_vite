...
import { radius, typography } from '@yleisradio/yds-core';
...
const getTagLinkBaseStyles = ({
  ...
  borderRadius: $removePadding && $variant === 'text' ? radius.SMALL : radius.FULL
}, fontStylesAsObject({
  theme,
  size: 'xs',
  modifier: FontStyleModifier.BOLD_AC
})), {
  letterSpacing: typography.DEFAULT_XS_BOLD_LETTER_SPACING,
  textDecoration: 'none',
  '-webkit-line-clamp': $numberOfLines > 0 ? `${$numberOfLines}` : '',
  '-webkit-box-orient': $numberOfLines > 0 ? 'vertical' : '',
  overflow: 'hidden'
});
const TagLinkBase = styled.a.withConfig({
...
const TagLinkBaseNonInteractive = styled.span.withConfig({
...
export { TagLinkBase, TagLinkBaseNonInteractive };
